...
async function buildRunpodPayload(input) {
  ...
  if (workflow === 'API_AVATAR_WF') {
    const selfieFile = files.find(f => ['selfie', 'fullshot'].includes(f.field));
    
    if (!selfieFile) {
      throw new Error('Selfie image is required for avatar workflow');
    }

    const uploadedFilename = await uploadFileToRunpod(selfieFile);

    return {
      input: {
        workflow_path: "/workspace/OUTFITZ/01-Workflows/API_AVATAR_WF.json",
        set_nodes: {
          "12.image_path": `/workspace/comfy/ComfyUI/input/${uploadedFilename}`,
